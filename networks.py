...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc=2, output_nc=1, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[]):
        #print(input_nc.size(), output_nc.size())
        assert(n_blocks >= 0)
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            #print('parallelllllldlfldlfldfldlfl',input.size())
